...
; Always require administrator elevation for install/uninstall.
RequestExecutionLevel admin

; Handle install mode, `perUser`, `perMachine` or `both`
!if "${INSTALLMODE}" == "perMachine"
 ; admin level set above
!endif

!if "${INSTALLMODE}" == "currentUser"
 ; admin level set above (installer always elevated)
!endif
...
; Installer pages — single combined setup page (Tracker custom UI)
Var TrackerDirField
Var TrackerDesktopBox
...
Function TrackerBrowseDir
 nsDialogs::SelectFolderDialog "Choose install folder" $INSTDIR
 Pop $0
 ${If} $0 == error
  Abort
 ${EndIf}
 ${NSD_SetText} $TrackerDirField $0
FunctionEnd

Function TrackerSinglePageLeave
 ${NSD_GetText} $TrackerDirField $INSTDIR
 ${NSD_GetState} $TrackerDesktopBox $0
 ${If} $0 == ${BST_CHECKED}
  StrCpy $TrackerDesktopState ${BST_CHECKED}
 ${Else}
  StrCpy $TrackerDesktopState ${BST_UNCHECKED}
 ${EndIf}
 ${NSD_GetState} $TrackerAutoUninstallBox $0
 ${If} $0 == ${BST_CHECKED}
...
; Reinstall / upgrade page (only when a previous install exists)
...
Function PageReinstall
 ...
 ; Default behaviour: update in place over the existing install (no uninstall).
 ; Only when the user ticked "Clean install" do we remove the old version first.
 ; Either way we skip the maintenance radio page entirely.
 ${If} $UpdateMode <> 1
  ${If} $TrackerAutoUninstallState == ${BST_CHECKED}
   Call PerformPreviousUninstall
  ${EndIf}
  Abort
 ${EndIf}
...
Function PerformPreviousUninstall
 HideWindow
 ClearErrors
...
; Installation progress (single-page flow — directory chosen on setup page)
!insertmacro MUI_PAGE_INSTFILES

; Finish — launch Tracker
