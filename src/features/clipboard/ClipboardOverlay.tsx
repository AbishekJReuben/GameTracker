...
export default function ClipboardOverlay() {
  ...
  const collapse = useCallback(async () => {
    await getCurrentWindow().setSize(new LogicalSize(BUBBLE, BUBBLE));
    setExpanded(false);
  }, []);
      ...
      <AnimatePresence mode="wait" initial={false}>
        {expanded ? (
          <motion.div
            ...
            className="glass flex h-full w-full flex-col overflow-hidden rounded-[22px] border border-white/12 shadow-float"
          ...
          <motion.button
            ...
            className="glass relative grid h-[68px] w-[68px] cursor-grab place-items-center rounded-full border border-white/15 shadow-float active:cursor-grabbing"
            style={{
              margin: 4,
