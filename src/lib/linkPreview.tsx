...
export function LinkPreview({ url }: { url: string }) {
  ...
  return <a href={data.url} onClick={(event) => { event.preventDefault(); void openExternalUrl(data.url); }} className="mt-2 flex select-text overflow-hidden rounded-xl border border-cyan-300/15 bg-cyan-300/[0.06] text-left transition-colors hover:border-cyan-200/35 hover:bg-cyan-300/[0.1]" title={`Open ${data.host}`}>
